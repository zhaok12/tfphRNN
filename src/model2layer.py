...
class AttentionTimRNN(nn.Module):
    def __init__(self, batch_size, num_tims, num_locs, embed_size_locs, embed_size_tims, tim_gru_hidden, bidirectional=True):
        ...
        if bidirectional:
            ...
            self.weight_W_tim = nn.Parameter(torch.rand(2 * tim_gru_hidden, 2 * tim_gru_hidden))
            self.bias_tim = nn.Parameter(torch.rand(2 * tim_gru_hidden, 1))
            self.weight_proj_tim = nn.Parameter(torch.rand(2 * tim_gru_hidden, 1))
        else:
            self.tim_gru = nn.GRU(embed_size_tims + embed_size_locs, tim_gru_hidden, bidirectional=False)
            self.weight_W_tim = nn.Parameter(torch.rand(tim_gru_hidden, tim_gru_hidden))
...
class AttentionWeekRNN(nn.Module):
    def __init__(self, batch_size, week_gru_hidden, tim_gru_hidden, week_nums, embed_size_week, bidirectional=True):
        ...
        if bidirectional:
            self.week_gru = nn.GRU(embed_size_week + 2*tim_gru_hidden, week_gru_hidden, bidirectional=True)
            self.weight_W_week = nn.Parameter(torch.Tensor(2 * week_gru_hidden, 2 * week_gru_hidden))
            self.bias_week = nn.Parameter(torch.Tensor(2 * week_gru_hidden, 1))
            self.weight_proj_week = nn.Parameter(torch.Tensor(2 * week_gru_hidden, 1))
            # self.final_linear = nn.Linear(2 * week_gru_hidden, n_classes)
        else:
            self.week_gru = nn.GRU(embed_size_week + tim_gru_hidden, week_gru_hidden, bidirectional=False)
            self.weight_W_week = nn.Parameter(torch.Tensor(week_gru_hidden, week_gru_hidden))
            self.bias_week = nn.Parameter(torch.Tensor(week_gru_hidden, 1))
            self.weight_proj_week = nn.Parameter(torch.Tensor(week_gru_hidden, 1))
    ...
    def forward(self, week_seq, week_vec, state_week):
        ...
        week_squish = batch_matmul_bias(output_week, self.weight_W_week, self.bias_week, nonlinearity='tanh')
        week_attn = batch_matmul(week_squish, self.weight_proj_week)
        week_attn_norm = self.softmax_week(week_attn.transpose(1, 0))
        week_attn_vectors = attention_mul(output_week, week_attn_norm.transpose(1, 0))
        return week_attn_vectors, state_week, week_attn_norm

    def init_hidden(self):
        if self.bidirectional:
            return Variable(torch.zeros(2, self.batch_size, self.week_gru_hidden))
        else:
            return Variable(torch.zeros(1, self.batch_size, self.week_gru_hidden))
...
def get_predictions(val_tokens, seq_week, tim_attn_model, week_attn_model, mlp_model):
    ...
    for user in range(2):
        ...
        for i in range(max_weeks):
            _s, state_tim, _ = tim_attn_model(val_tokens[user, i, 1, :, :].transpose(0, 1),
...
def pad_batch(mini_batch):
    ...
    main_matrix = np.zeros((mini_batch_size, 2, max_week_len, max_tim_len, 2), dtype=np.int)
            ...
            for j in range(main_matrix.shape[2]):
                try:
                    ...
                    for p in range(main_matrix.shape[3]):
                            main_matrix[i, user, j, p, 0] = mini_batch[i][user][weeks[j]][p][0]  # loc
                            main_matrix[i, user, j, p, 1] = mini_batch[i][user][weeks[j]][p][1]  # time
                except IndexError:
                    pass
    return Variable(torch.from_numpy(main_matrix).permute(1, 2, 4, 0, 3)), Variable(torch.from_numpy(week_matrix).permute(1, 0, 2))
...
def test_accuracy_full_batch(tokens, labels, mini_batch_size, tim_attn, week_attn, mlp):
    p = []
    l = []
    g = gen_minibatch(tokens, labels, mini_batch_size)
    for token, seq_week, seq_day, label in g:
        y_pred = get_predictions(tokens.cuda(), seq_week, tim_attn, week_attn, mlp)
        _, y_pred = torch.max(y_pred, 1)
...
def gen_minibatch(tokens, labels, mini_batch_size, shuffle=True):
    for token, label in iterate_minibatches(tokens, labels, mini_batch_size, shuffle=shuffle):
        ...
        yield token.cuda(), seq_week.cuda(), Variable(torch.from_numpy(label), requires_grad=False).float().cuda()


def check_val_loss(val_tokens, val_labels, mini_batch_size, tim_attn_model, week_attn_model, mlp_model, criterion):
    val_loss = []
    for token, label in iterate_minibatches(val_tokens, val_labels, mini_batch_size, shuffle=True):
        token, seq_week = pad_batch(token)
        y_pred = get_predictions(token.cuda(), seq_week, tim_attn_model, week_attn_model, mlp_model)
        loss = criterion(y_pred.cuda(), Variable(torch.from_numpy(label), requires_grad=False).float().cuda())
...
def train_data(mini_batch, seq_week, targets, tim_attn_model, week_attn_model, mlp_model,
               tim_optimizer, week_optimizer, mlp_optimizer, criterion):
    state_tim = tim_attn_model.init_hidden().cuda()
    state_week = week_attn_model.init_hidden().cuda()
    ...
    for user in range(2):
        s1 = None
        for i in range(max_weeks):
            _s, state_tim, _ = tim_attn_model(mini_batch[user, i, 1, :, :].transpose(0, 1),
...
# Training
def train_early_stopping(mini_batch_size, X_train, y_train, X_test, y_test, tim_attn_model, week_attn_model, mlp_model,
                         tim_attn_optimiser, week_attn_optimiser, mlp_optimizer, loss_criterion, num_epoch,
    ...
    for i in range(1, num_epoch + 1):
        try:
            ...
            loss = train_data(tokens, seq_week, labels, tim_attn_model, week_attn_model, mlp_model, tim_attn_optimiser,
                              week_attn_optimiser, mlp_optimizer, loss_criterion)
            acc = test_accuracy_mini_batch(tokens, seq_week, labels, tim_attn_model, week_attn_model, mlp_model)
            ...
            if i % print_val_loss_every == 0:
                val_loss = check_val_loss(X_test, y_test, mini_batch_size, tim_attn_model, week_attn_model, mlp_model, loss_criterion)
...
def train():
    ...
    tim_attn = AttentionTimRNN(batch_size=mini_batch_size, num_tims=168, num_locs=40559, embed_size_locs=embed_size_locs,
                               embed_size_tims=embed_size_tims, tim_gru_hidden=tim_gru_hidden, bidirectional=True)
    week_attn = AttentionWeekRNN(batch_size=mini_batch_size, week_gru_hidden=week_gru_hidden, tim_gru_hidden=tim_gru_hidden,
                                 week_nums=133, embed_size_week=embed_size_week, bidirectional=True)
    mlp = MLPNet(input_size=2*week_gru_hidden, hidden_size=[50, 30], bidirectional=True)

    learning_rate = 1e-1
    momentum = 0.7
    tim_optmizer = torch.optim.SGD(tim_attn.parameters(), lr=learning_rate, momentum=momentum)
    week_optimizer = torch.optim.SGD(week_attn.parameters(), lr=learning_rate, momentum=momentum)
    ...
    loss_full = train_early_stopping(mini_batch_size, X_train, y_train, X_test, y_test, tim_attn, week_attn, mlp,
                                     tim_optmizer, week_optimizer, mlp_optimizer,
